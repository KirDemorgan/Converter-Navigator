...
## 8a. Тест-политика (обязательна)

Каждая фича едет с тестами в том же коммите. Без тяжёлых интеграционников/запуска IDE.
Двухуровневый подход:
- **Чистые юниты (JUnit, без платформы)** — на всю логику, отделимую от PSI. Поэтому
  логику ВЫНОСИМ в PSI-free функции: `NameRule` (парсинг имени), `ConversionMatching`
  (порядок дженериков, матч тип↔конверсия, игнор java.lang). Мгновенные.
- **Light-фикстуры (`LightJavaCodeInsightFixtureTestCase`)** — на PSI-зависимое: детектор
  на реальных Java/Kotlin классах, line markers через `myFixture`. Headless, без GUI,
  секунды. Стабы внешних либ (Spring `Converter`, MapStruct `@Mapper`) добавляем прямо
  в тест через `myFixture.addClass` — реальные зависимости не нужны.
- Запуск: `./gradlew test`.
